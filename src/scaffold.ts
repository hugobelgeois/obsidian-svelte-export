...
/**
 * Runs a shell command without blocking Obsidian's renderer thread — unlike
 * `execSync`, which freezes the entire app (not just this plugin) for as
 * long as `npx sv create`/`npm install` take, since Obsidian plugins run in
 ...
 *
 * `onStatus`, if given, is called with a short human-readable label at each
 * step — used to drive the export progress modal, since this phase can take
 * a while on the first export (project scaffold + npm install).
 *
 * Returns `false` if initialisation failed (caller should abort the export).
 */
export async function ensureSvelteProject(
	destRoot: string,
	pluginDir: string,
	vaultPath: string,
	configDir: string,
	...
	if (!isSvelteProject) {
		onStatus?.("Setting up SvelteKit project…");
		...
		try {
			...
			await runCommand("npm install", destRoot);
	...
	copyPluginFiles(destRoot, pluginDir, vaultPath, configDir, selectedTheme);
	...
	await ensureGitHubPagesDeployment(destRoot, onStatus);
 ...
 */
async function ensureGitHubPagesDeployment(
	destRoot: string,
	onStatus?: (message: string) => void,
): Promise<void> {
	await patchAdapterForStaticHosting(destRoot, onStatus);
 ...
 */
async function patchAdapterForStaticHosting(
	destRoot: string,
	onStatus?: (message: string) => void,
): Promise<void> {
	const viteConfigPath = path.join(destRoot, "vite.config.ts");
	...
	if (!hasAdapterStatic) {
		try {
			onStatus?.("Installing @sveltejs/adapter-static…");
			new Notice(
				"📦 installing @sveltejs/adapter-static for GitHub Pages…",
			);
			await runCommand("npm install -D @sveltejs/adapter-static", destRoot);
		} catch (e) {
			console.error(
...
function copyPluginFiles(
	...
	const obsidianDir = path.join(vaultPath, configDir);
...
function resolveActiveThemeAndSnippets(
	...
	if (fs.existsSync(appearancePath)) {
		try {
			...
			) as { cssTheme?: string; enabledCssSnippets?: string[] };
